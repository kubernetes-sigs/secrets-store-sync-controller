...
const syncWaitTimeout = 300 * time.Second
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			expectedCondition: &metav1.Condition{
				...
				Message: "failed to get SecretProviderClass \"e2e-providerspc\": secretproviderclass.secrets-store.csi.x-k8s.io \"e2e-providerspc\" not found",
			},
		},
	} {
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			expectedCondition: &metav1.Condition{
				...
				Status:  metav1.ConditionFalse,
				Reason:  "SecretProviderClassMisconfigured",
				Message: "failed to get SecretProviderClass \"not-very-random-but-will-do\": secretproviderclass.secrets-store.csi.x-k8s.io \"not-very-random-but-will-do\" not found",
			},
		},
	} {
		f.RunTest(t, tc.name, func(t *testing.T, testCfg *e2elib.TestConfig) {
