...
BATS_TESTS_DIR=hack/localsetup
WAIT_TIME=60
SLEEP_TIME=1
...
@test "[v1alpha1] deploy e2e-providerspc secretproviderclass crd" {
  ...
  envsubst <  $BATS_TESTS_DIR/e2e-providerspc.yaml | kubectl apply -n test-v1alpha1 -f -
...
@test "[v1alpha1] deploy e2e-providerspc secretsync crd" {
  ...
  envsubst < $BATS_TESTS_DIR/e2e-secret-sync.yaml | kubectl apply -n test-v1alpha1 -f -
...
@test "SecretProviderClass and SecretSync are deployed in different namespaces" {
  ...
  envsubst < $BATS_TESTS_DIR/e2e-providerspc.yaml | kubectl apply -n spc-namespace -f -
  ...
  envsubst < $BATS_TESTS_DIR/e2e-secret-sync.yaml | kubectl apply -n ss-namespace -f -
