...
archive_info() {
  ...
  #
  # assumes driver is installed with helm into the `secret-sync-controller-system` namespace which
  # sets the `app.kubernetes.io/created-by` selector to `secret-sync-controller`.
  #
  # Note: the yaml deployment would require `app=csi-secrets-store`
  kubectl logs -l app.kubernetes.io/created-by=secret-sync-controller  --tail -1 -c secrets-store -n secret-sync-controller-system > ${LOGS_DIR}/secrets-store.log
  kubectl logs -l app.kubernetes.io/created-by=secret-sync-controller  --tail -1 -c liveness-probe -n secret-sync-controller-system > ${LOGS_DIR}/liveness-probe.log
  ...
  for pod_ip in $(kubectl get pod -n kube-system -l app.kubernetes.io/created-by=secret-sync-controller -o jsonpath="{.items[*].status.podIP}")
  do
    kubectl exec -n default ${curl_pod_name} -- curl -s http://${pod_ip}:8080/metrics > ${LOGS_DIR}/${pod_ip}.metrics
  done
...
get_secret_sync_api_version() {
  local api_version=$(kubectl api-resources --api-group='secret-sync.x-k8s.io' --no-headers=true | awk '{ print $2 }' | uniq)
  echo "${api_version}"
}
