...
func (r *SecretSyncReconciler) Run(ctx context.Context, workers int, providerPollingPeriod time.Duration) error {
	...
	if providerPollingPeriod > 0 {
		go wait.UntilWithContext(ctx, r.providerPollingFunc, providerPollingPeriod)
	}
...
func (r *SecretSyncReconciler) processNextWorkItem(ctx context.Context) bool {
	...
	ctx = klog.NewContext(ctx, logger.WithValues("objectRef", objRef.String()))
...
// FIXME: rewrite to something more client-go specific
func (r *SecretSyncReconciler) providerPollingFunc(ctx context.Context) {
	logger := klog.FromContext(ctx)
	for {
		ssList, err := r.secretProviderClassLister.List(labels.Everything())
		if err != nil {
			logger.Error(err, "failed to list SecretSyncs")
			continue
		}
		for secretSync := range ssList {
			r.enqueue(secretSync)
		}
	}
}
