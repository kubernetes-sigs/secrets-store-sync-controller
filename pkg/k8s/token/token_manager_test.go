...
func TestDeleteServiceAccountToken(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			failGetToken := func(_, _ string, _ *authenticationv1.TokenRequest) (*authenticationv1.TokenRequest, error) {
			...
			for _, index := range c.requestIndex {
				...
				_, _ = testMgr.GetServiceAccountToken(req.namespace, req.name, &req.tr)
			}
