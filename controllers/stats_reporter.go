...
const (
	...
	CreateUpdateVAP               = "secret-sync-controller-create-update-policy"
	CreateUpdateTokenDenyVAP      = "secret-sync-controller-create-update-token-deny-policy"
	UpdateLabelVAP                = "secret-sync-controller-update-label-policy"
	UpdateOwnersCheckOldObjectVAP = "secret-sync-controller-update-owners-check-oldobject-policy"
	ValidateTokenVAP              = "secret-sync-controller-validate-token-policy"
)
...
func NewStatsReporter() (StatsReporter, error) {
	...
	if r.secretSyncDuration, err = meter.Float64Histogram("secret_sync_duration_sec", metric.WithDescription("Distribution of how long it took to sync k8s secret")); err != nil {
		return nil, err
	}

	if r.SSCCreateUpdateVAPTotal, err = meter.Int64Counter("ssc_create_update_vap_total", metric.WithDescription("Total number of secret-sync-controller-create-update-policy denials")); err != nil {
		return nil, err
	}

	if r.SSCCreateUpdateTokenDenyVAPTotal, err = meter.Int64Counter("ssc_create_update_token_deny_vap_total", metric.WithDescription("Total number of secret-sync-controller-create-update-token-deny-policy denials")); err != nil {
		return nil, err
	}

	if r.SSCUpdateLabelVAPTotal, err = meter.Int64Counter("ssc_update_label_vap_total", metric.WithDescription("Total number of secret-sync-controller-update-label-policy denials")); err != nil {
		return nil, err
	}

	if r.SSCUpdateOwnersCheckOldObjectVAPTotal, err = meter.Int64Counter("ssc_update_owners_check_oldobject_vap_total", metric.WithDescription("Total number of secret-sync-controller-update-owners-check-oldobject-policy denials")); err != nil {
		return nil, err
	}

	if r.SSCValidateTokenVAPTotal, err = meter.Int64Counter("ssc_validate_token_vap_total", metric.WithDescription("Total number of secret-sync-controller-validate-token-policy denials")); err != nil {
		return nil, err
	}
...
// recordVAPDeny records the VAP deny metric
