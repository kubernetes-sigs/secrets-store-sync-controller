...
func (r *SecretSyncReconciler) prepareLabelsAndAnnotations(
	...
	if val, ok := labels[ControllerLabelKey]; ok && len(val) > 0 {
		...
		return nil, nil, fmt.Errorf("label %s is reserved for use by the secret sync controller", ControllerLabelKey)
	}
	...
	if val, ok := annotations[ControllerAnnotationKey]; ok && len(val) > 0 {
		...
		return nil, nil, fmt.Errorf("annotation %s is reserved for use by the secret sync controller", ControllerAnnotationKey)
	}
